# 📄 文件 2：`sop/FIELD_MAINTENANCE_SOP.md`
...
**工作原理**：
1. 用户在前端勾选 `f1s9DJg4oLc`（公开地区）
2. 系统检查该字段值 → `true` 则显示 `displayIds` 中的字段，否则隐藏
...
> 💡 这是一个**快速查找表**，用于 `filterFieldsByRoleAndPrivacy` 函数快速判断某字段是否受隐私控制。

### 3.3 `ROLE_FIELD_VISIBILITY` —— 角色权限白名单

```javascript
ROLE_FIELD_VISIBILITY: {
  guest: ['fqidTsBW5js', 'fxwUAnrwpaT'],  // 游客仅看姓名、编号
  self: ['fqidTsBW5js', 'fxwUAnrwpaT', 'f3sJj8pDk2E'],
  verified: ['fqidTsBW5js', 'fxwUAnrwpaT', 'f3sJj8pDk2E', 'fwK2mQMoxto'],
  subadmin: ['fqidTsBW5js', 'fxwUAnrwpaT', 'f3sJj8pDk2E', 'fwK2mQMoxto', 'fgerzjJpBTF'],
  admin: ['*'],  // ★ '*' 表示拥有所有字段权限，管理员不受限制
}
```
...
| 游客 | `guest` | 未登录用户，权限最低 |
| 普通用户 | `self` | 已注册但未认证 |
| 认证用户 | `verified` | 已通过认证（管理员可认证） |
| 次级管理 | `subadmin` | 可管理档案但不能修改用户角色 |
| 根源管理 | `admin` | 最高权限，可看所有字段 |

**维护场景**：

| 操作 | 修改内容 | 示例 |
| :--- | :--- | :--- |
| 某字段仅对认证用户可见 | 在 `guest` 和 `self` 的数组中删除该 ID | — |
| 放宽某字段至游客可见 | 在 `guest` 的数组中添加该 ID | — |
...
| 你想做什么 | 涉及文件 | 具体操作 |
| :--- | :--- | :--- |
| **新增普通展示字段** | `instances.js` | ① `FIELD_LABELS` 加一行<br>② `DETAIL_GROUPS` 对应分组加 ID<br>③ 可选：`CARD_FIELDS` 加映射<br>④ 可选：`SEARCH_FIELDS` 加 ID |
...
| **新增隐私控制** | `instances.js` | ① `PRIVACY_RULES` 加规则<br>② `PRIVACY_DEPENDENCIES` 加依赖 |
| **调整角色权限** | `instances.js` | 修改 `ROLE_FIELD_VISIBILITY` 对应角色数组 |
| **新增可切换的隐私字段** | `instances.js` + `admin.js` | ① 按"新增隐私控制"操作<br>② `admin.js` 的 `SUB_EDITABLE_FIELDS` 加该字段 ID |
| **带单位的数值字段** | `instances.js` + `detail.js` | ① 按"新增普通展示字段"操作<br>② `detail.js` 的 `switch` 中新增 `case` |
| **星级评分字段** | `instances.js` + `detail.js` | ① 按"新增普通展示字段"操作<br>② `detail.js` 的 `if` 条件中加入该 ID |
| **更换 Fillout 表** | `api.js` + `instances.js` | ① `api.js` 改 `DATABASE_ID` / `TABLE_ID`<br>② `instances.js` 全部字段 ID 替换为新表 ID |
...
| 维护类型 | 文件数 | 核心文件 |
| :--- | :--- | :--- |
| 日常字段迭代 | **1 个** | `instances.js` |
| 涉及管理面板 | **2 个** | `instances.js` + `admin.js` |
| 涉及特殊格式（单位/星级） | **2 个** | `instances.js` + `detail.js` |
| 涉及数据源切换 | **2 个** | `api.js` + `instances.js` |
| 涉及权限/隐私逻辑变更 | **1~2 个** | `instances.js` ± `utils.js` |
